...
// C언어에서는 관례적으로 함수의 리턴을 사용하여 종료 코드를 반환합니다.
// 0: 함수 호출이 성공한 경우(오류가 발생하지 않은 경우)
// 0이외의 나머지 -> 일반적으로 -1을 사용: 
// -> 함수 호출이 실패한 경우(오류가 발생한 경우)
int div(int a, int b) {
	if (b == 0)
		return -1;
	return a / b;
...
#include<stdio.h>
// step 2. 이전의 코드에서는 함수의 리턴에 결과와 오류 코드가
//혼재되어 있다는 문제가 있습니다. 
//이를 해결하기 위해 결과와 오류코드를 분리합니다.
...
#include<stdio.h>
// step 3. 이전의 코드에서는 함수가 오류 코드 
//또는 에러 정보를 반환 하는지 알 수 없다는
// 단점이 있습니다. 이를 해결하기 위해 typedef를 사용합니다.

typedef int error_t;
error_t div(int a, int b, int* out) {	
	//코드 자체가 주석이 되는것이 중요합니다.
	//실패한 경우
	if (b == 0 || out == NULL)
		return -1;
	//성공한 경우
